...
type RedisKafkaMessage struct {
	Before model.Video `json:"before"`
	After  model.Video `json:"after"`
	Op     string      `json:"op"`
}

func NewRedisKafkaReader(log *log.Helper, k *conf.Kafka, rdb *redis.Client) {
	broker := k.Broker
...
func InitRedisKafkaConsumer(ctx context.Context, log *log.Helper, reader *kafka.Reader, rdb *redis.Client) {
	...
	for { //消息队列里随时可能有新消息进来，所以这里是死循环，类似于读Channel
		if message, err := reader.ReadMessage(ctx); err != nil {
			...
			if id != 0 {
				//删除缓存
				err = rdb.Del(ctx, fmt.Sprintf("videoInfo::%d", id)).Err()
				if err != nil {
